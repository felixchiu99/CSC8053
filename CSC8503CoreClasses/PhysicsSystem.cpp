...
#include "../CSC8503/PlayerController.h"
...
void PhysicsSystem::ForceAllAwake() {
	...
	for (auto i = first; i != last; ++i) {
		...
		(*i)->SetAwake(true);
		(*i)->GetPhysicsObject()->ResetAwakeCounter();

	}
}

void PhysicsSystem::UpdateOriginalAwake() {
	std::vector<GameObject*>::const_iterator first;
	std::vector<GameObject*>::const_iterator last;
	gameWorld.GetObjectIterators(first, last);
	...
	for (auto i = first; i != last; ++i) {
		...
		(*i)->GetPhysicsObject()->SetOriginalPosition(t.GetPosition());
		(*i)->GetPhysicsObject()->SetOriginalAngularVelocity((*i)->GetPhysicsObject()->GetLinearVelocity());

	}
}

void PhysicsSystem::PutToSleep(float dt) {
	int count = 0;
	std::vector<GameObject*>::const_iterator first;
	std::vector<GameObject*>::const_iterator last;
	gameWorld.GetObjectIterators(first, last);
	...
	for (auto i = first; i != last; ++i) {
		...
		if ((*i)->KeepAwake())
			continue;
		Transform& t = (*i)->GetTransform();
		Vector3 linearDiff = (*i)->GetPhysicsObject()->GetOriginalPosition() - t.GetPosition();
		Vector3 angularDiff = (*i)->GetPhysicsObject()->GetOriginalAngularVelocity()* (*i)->GetPhysicsObject()->GetAngularVelocity();
		//std::cout << linearDiff.LengthSquared() << " : " << (*i)->GetPhysicsObject()->GetLinearVelocity().LengthSquared() <<std::endl;
		if (linearDiff.LengthSquared() < 0.01* dt && angularDiff.LengthSquared() < 0.00001 * dt) {
			(*i)->GetPhysicsObject()->IncrementAwakeCounter();
		}
		else {
			(*i)->GetPhysicsObject()->ResetAwakeCounter();
		}
		if ((*i)->GetPhysicsObject()->GetAwakeCounter() > 20) {
			(*i)->GetPhysicsObject()->ResetAwakeCounter();
			(*i)->SetAwake(false);
		}

	}
	//std::cout << "awake count : " << count << std::endl;
}
...
void PhysicsSystem::UpdateCollisionList() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = allCollisions.begin(); i != allCollisions.end(); ) {
		if ((*i).framesLeft == numCollisionFrames) {
			...
			int p = (*i).point.penetration;
			std::map<GameObject*, bool> col = i->a->GetCollidedWith();
			std::map<GameObject*, bool>::iterator it;

			bool calScore = true;
			if (
				i->a->GetObjectType() == ObjectType::Player || 
				i->b->GetObjectType() == ObjectType::Player
				) {
				calScore = false;
			}
			if (
				i->a->GetObjectType() == ObjectType::AI &&
				i->b->GetObjectType() == ObjectType::Inmobile
				) {
				calScore = false;
			}
			if (
				i->b->GetObjectType() == ObjectType::AI &&
				i->a->GetObjectType() == ObjectType::Inmobile
				) {
				calScore = false;
			}
			float velDiff = (i->a->GetPhysicsObject()->GetLinearVelocity() - i->b->GetPhysicsObject()->GetLinearVelocity()).Length();
			if (velDiff > 2 && calScore) {
				for (it = col.begin(); it != col.end(); it++)
				{
					((PlayerController*)it->first)->AddScore(velDiff);
				}
			}
		}
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	float impulseForse = Vector3::Dot(contactVelocity, p.normal);
	...
	float cRestitutionA = physA->GetElasticity();// disperse some kinectic energy
	float cRestitutionB = physB->GetElasticity();// disperse some kinectic energy

	//float j = (-(1.0f + cRestitution) * impulseForse) / (totalMass + angularEffect);
	float jA = (-(1.0f + cRestitutionA) * impulseForse) / (totalMass + angularEffect);
	float jB = (-(1.0f + cRestitutionB) * impulseForse) / (totalMass + angularEffect);
...
void PhysicsSystem::NarrowPhase() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator
		...
		//(info.a)->GetRenderObject()->SetColour(Vector4(0, 0, 1, 1));
		//(info.b)->GetRenderObject()->SetColour(Vector4(1, 0, 0, 1));
