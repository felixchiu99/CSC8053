...
bool CollisionDetection::ObjectWithinRange(GameObject* a, GameObject* b , float dist, CollisionInfo& collisionInfo) {
	const SphereVolume* volume = new SphereVolume(dist);
	const CollisionVolume* volA = (CollisionVolume*)volume; // self
	const CollisionVolume* volB = b->GetBoundingVolume(); // target
	...
	if (pairType == VolumeType::Sphere) {
		inRange = SphereIntersection((SphereVolume&)* volA, transformA, (SphereVolume&)* volB, transformB, collisionInfo);
	}

	//AABB vs Sphere pairs
	if (volA->type == VolumeType::AABB && volB->type == VolumeType::Sphere) {
		inRange = AABBSphereIntersection((AABBVolume&)* volA, transformA, (SphereVolume&)* volB, transformB, collisionInfo);
	}
	if (volA->type == VolumeType::Sphere && volB->type == VolumeType::AABB) {
		collisionInfo.a = b;
		collisionInfo.b = a;
		inRange = AABBSphereIntersection((AABBVolume&)* volB, transformB, (SphereVolume&)* volA, transformA, collisionInfo);
	}

	//OBB vs sphere pairs
	if (volA->type == VolumeType::OBB && volB->type == VolumeType::Sphere) {
		inRange = OBBSphereIntersection((OBBVolume&)* volA, transformA, (SphereVolume&)* volB, transformB, collisionInfo);
	}
	if (volA->type == VolumeType::Sphere && volB->type == VolumeType::OBB) {
		collisionInfo.a = b;
		collisionInfo.b = a;
		inRange = OBBSphereIntersection((OBBVolume&)* volB, transformB, (SphereVolume&)* volA, transformA, collisionInfo);
	}

	//Capsule vs other interactions
	if (volA->type == VolumeType::Capsule && volB->type == VolumeType::Sphere) {
		inRange = SphereCapsuleIntersection((CapsuleVolume&)* volA, transformA, (SphereVolume&)* volB, transformB, collisionInfo);
	}
	if (volA->type == VolumeType::Sphere && volB->type == VolumeType::Capsule) {
		collisionInfo.a = b;
		collisionInfo.b = a;
		inRange = SphereCapsuleIntersection((CapsuleVolume&)* volB, transformB, (SphereVolume&)* volA, transformA, collisionInfo);
	}
...
Ray CollisionDetection::BuildRayFromCentre(const Camera& cam) {
	Vector2 screenMouse = Window::GetWindow()->GetScreenSize() / 2;
	Vector2 screenSize = Window::GetWindow()->GetScreenSize();
