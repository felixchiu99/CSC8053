...
bool NavigationGrid::FindPath(const Vector3& from, const Vector3& to, NavigationPath& outPath) {
	...
	int fromX = (((int)from.x + nodeSize * 0.5) / nodeSize);
	int fromZ = (((int)from.z + nodeSize * 0.5) / nodeSize);

	int toX = (((int)to.x + nodeSize * 0.5) / nodeSize);
	int toZ = (((int)to.z + nodeSize * 0.5) / nodeSize);
