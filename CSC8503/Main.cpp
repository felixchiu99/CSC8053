...
void TestBehaviourTree() {
	...
	for (int i = 0; i < 5; ++i) {
		...
		if (state == Success) {
			std::cout << "what a success!\n";
		}
		else if (state == Failure) {
			std::cout << "what a waste of time!\n";
		}
	}
	std::cout << "TestBehaviourTree DONE\n";
}
...
class GameScreen : public PushdownState {
	...
	PushdownResult OnUpdate(float dt, PushdownState** newState) override {
		...
		if (Window::GetKeyboard()->KeyPressed(KeyboardKeys::Y)) {
			w->SetWindowPosition(600, 400);
		}

		w->SetTitle(NetworkedGame01.GetName() + " : Gametech frame time:" + std::to_string(1000.0f * dt));

		NetworkedGame01.UpdateGame(dt);
...
public:
	...
	void ReceivePacket(int type, GamePacket* payload, int source) {
		...
		if (type == String_Message) {
			...
			std::cout << name  << " ID " << source << " received message: " << msg << std::endl;
		}
...
int main() {
	...
	NetworkedGame NetworkedGame01 = NetworkedGame();
	//NetworkedGame NetworkedGameClient01 = NetworkedGame();

	//NetworkedGameServer.SetName("Server");
	//NetworkedGameServer.StartAsServer();

	//NetworkedGameClient01.SetName("Client01");
	//NetworkedGameClient01.StartAsClient(127, 0, 0, 1);
	...
	while (w->UpdateWindow() && !Window::GetKeyboard()->KeyDown(KeyboardKeys::ESCAPE)) {
		...
		w->SetTitle(NetworkedGame01.GetName() + " : Gametech frame time: " + std::to_string(dt) + " estim fps: " + std::to_string(60/dt*1000));
