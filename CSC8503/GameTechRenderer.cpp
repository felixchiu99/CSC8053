...
GameTechRenderer::GameTechRenderer(GameWorld& world) : OGLRenderer(*Window::GetWindow()), gameWorld(world)	{
	...
	glGenBuffers(1, &vertexColourVBO);
...
void GameTechRenderer::RenderCamera() {
	...
	for (const auto&i : activeObjects) {
		...
		bool hasVertexColour = !(i)->GetVertexColours().empty();
		glUniform1i(hasVColLocation, hasVertexColour);

		if (hasVertexColour) {
			// the second VBO, the color
			glBindBuffer(GL_ARRAY_BUFFER, vertexColourVBO);
			glEnableVertexAttribArray(4);
			glBufferData(GL_ARRAY_BUFFER, sizeof(float) * (i)->test.size(), nullptr, GL_DYNAMIC_DRAW);
			glVertexAttribPointer(
				4,                 // location = 4 in verties shader file
				1,                 // the position has R,G,B,A 4 elements
